...
class SeedDMS_Bootstrap_Style extends SeedDMS_View_Common {
	...
	function htmlStartPage($title="", $bodyClass="", $base="") { /* {{{ */
		...
		echo '<link href="/styles/'.$this->theme.'/dist/css/skins/skin-blue.min.css" rel="stylesheet">'."\n";
...
//		$content .= "    <li><a href=\"/op/op.ClearClipboard.php?refferer=".urlencode($this->params['refferer'])."\">".getMLText("clear_clipboard")."</a><a class=\"ajax-click\" data-href=\"/op/op.Ajax.php\" data-param1=\"command=clearclipboard\">kkk</a> </li>\n";
    ...
    echo "<span class=\"logo-mini\"><b>MSEI</b></span>"; // TODO: change for mini logo 
...
//			$('.delete-document-btn').click(function(ev) {
	...
	function printDeleteFolderButton($folder, $msg, $return=false){ /* {{{ */
		$folderid = $folder->getID();
		$content = '';
		$content .= '<a type="button" class="btn btn-danger btn-sm delete-folder-btn" rel="'.$folderid.'" msg="'.getMLText($msg).'" confirmmsg="'.htmlspecialchars(getMLText("confirm_rm_folder", array ("foldername" => $folder->getName())), ENT_QUOTES).'"><i class="fa fa-times"></i></a>';
		if($return)
			return $content;
...
//		$content .= "<td></td>";
		...
		if($enableClipboard) {
			$content .= '<a type="button" class="btn btn-default btn-sm addtoclipboard" rel="F'.$subFolder->getID().'" msg="'.getMLText('splash_added_to_clipboard').'" title="'.getMLText("add_to_clipboard").'"><i class="fa fa-clone"></i></a>';
		}
		$content .= "</div>";
		$content .= "</td>";
		$content .= "</tr>\n";
